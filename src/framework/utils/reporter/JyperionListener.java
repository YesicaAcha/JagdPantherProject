package framework.utils.reporter;
...
import framework.utils.TakeSnapshot;
...
public class JyperionListener implements ITestListener {
	...
	public void onTestFailure(ITestResult result) {
		...
		String file = "C:\\"+"screenshot"+(new Random().nextInt())+".png";
		try {
			TakeSnapshot.takeSnapShot(SeleniumDriverManager.getManager().getDriver(), file);
		} catch (Exception e) {
	...
	public void onStart(ITestContext context) {
		...
		Paragraph p = new Paragraph(context.getName() + " TESTNG RESULTS :)",
				FontFactory.getFont(FontFactory.HELVETICA, 20, Font.BOLD, new Color(255, 0, 0)));
	...
	public void onFinish(ITestContext context) {
		...
		Paragraph p = new Paragraph("EXCEPTIONS SUMMARY",
				FontFactory.getFont(FontFactory.HELVETICA, 14, Font.ITALIC, new Color(0, 0, 255)));
		try {
			this.document.add(p);
		} catch (DocumentException e1) {
			e1.printStackTrace();
